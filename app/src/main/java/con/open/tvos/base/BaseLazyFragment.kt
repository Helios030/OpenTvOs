...
abstract class BaseLazyFragment : Fragment(), CustomAdapt {
    ...
    private var mLoadService: Any? = null
    ...
    fun <T : View> findViewById(@IdRes viewId: Int): T? {
        return rootView?.findViewById(viewId) as? T
    }
    ...
    protected fun showLoading() {
        (mLoadService as? com.kingja.loadsir.core.LoadService)?.showCallback(LoadingCallback::class.java)
    }

    protected fun showEmpty() {
        (mLoadService as? com.kingja.loadsir.core.LoadService)?.showCallback(EmptyCallback::class.java)
    }

    protected fun showSuccess() {
        (mLoadService as? com.kingja.loadsir.core.LoadService)?.showSuccess()
    }
    ...
    override fun getSizeInDp(): Float {
        return if (activity is CustomAdapt) {
            (activity as CustomAdapt).sizeInDp
        } else {
            0f
        }
    }
    ...
    override fun isBaseOnWidth(): Boolean {
        return if (activity is CustomAdapt) {
            (activity as CustomAdapt).isBaseOnWidth
        } else {
            true
        }
    }
